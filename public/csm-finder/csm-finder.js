...
(function () {
	...
	function init() {
		...
		var mapBtn = el("button", {
			...
			"data-view": "map",
			innerHTML:
				'<span class="c-csm-finder__view-btn-icon">' + ICONS.map + "</span>Map",
		});
		...
		if (!hasLeaflet) {
			...
			mapBtn.title = "Map view requires JavaScript and Leaflet to be available";
		...
		function updateStatus() {
			...
			if (state.query) {
				...
				statusEl.textContent =
					count + " community safety manager" + (count !== 1 ? "s" : "");
			}
		}
